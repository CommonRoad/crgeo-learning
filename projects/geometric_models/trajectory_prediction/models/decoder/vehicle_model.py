...
from crgeo.common.torch_utils.helpers import assert_size
...
class RelativePositionAndOrientationVehicleModel(VehicleModel[RelativePositionAndOrientationVehicleStates]):
    ...
    def compute_single_step_loss(
        ...
        position_loss = loss_fn(prediction.position, target.position)
        orientation_loss = loss_fn(torch.zeros_like(prediction.orientation), relative_orientation(prediction.orientation, target.orientation))
        # print(position_loss, orientation_loss)
        return {
            "primary_weighted": sum((
                loss_fn(prediction.position, target.position),
                orientation_weight * loss_fn(torch.zeros_like(prediction.orientation), relative_orientation(prediction.orientation, target.orientation)),
            )),
            "primary": sum((
                loss_fn(prediction.position, target.position),
                loss_fn(torch.zeros_like(prediction.orientation), relative_orientation(prediction.orientation, target.orientation)),
