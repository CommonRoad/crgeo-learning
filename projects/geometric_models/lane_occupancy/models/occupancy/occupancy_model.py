...
from crgeo.common.torch_utils.pygeo import get_batch_internal_indices
...
class OccupancyModel(BaseGeometric):
    ...
    def train_preprocess(
        ...
        from crgeo.common.torch_utils.pygeo import get_batch_sizes, get_batch_start_indices
    ...
    def compute_loss(
        ...
        **kwargs
        ...
        info = dict(
            ...
            #wl2=w_l2.item(),
            #dta=delta_loss.item(),
            #mstp=occ_probs.std(axis=2).mean().item(),
            #sstp=occ_probs.std(axis=2).std().item(),
            #avp=occ_probs.mean(axis=2).mean().item(),
            #occ=y_disc.mean().item(),
            #mtn=mean_theta_norm.item(),
        ...
        if self.config.variational:
            info['kld'] = kld_loss.item()
        if not self.config.path_conditioning or self.config.loss.path_auxillary_loss:
            info.update(dict(
                pl=nll_p.mean().item(),
                nl=nll_n.mean().item(),
                mp=positive_occ_probs.mean().item(),
                mn=negative_occ_probs.mean().item(),
            ))
        if self.config.path_conditioning:
            info.update(dict(
                plp=nll_p_path.mean().item(),
                nlp=nll_n_path.mean().item(),
                mp=positive_occ_probs_path.mean().item(),
                mn=negative_occ_probs_path.mean().item(),
