...
from crgeo.common.threading.delayed_executor import DelayedExecuter
...
if TYPE_CHECKING:
    from crgeo_learning.reinforcement.commonroad_gym_env import CommonRoadGymEnv
...
class _EpisodeResetter(Generic[T_BaseSimulationOptions], AutoReprMixin):
    ...
    def __init__(
        ...
        self._delayed_executer = DelayedExecuter(
    ...
    def on_reset(self) -> None:
        ...
        if self._async_resets:
            ...
            if not ready_on_time:
                self._delayed_executer.disable()
            if self._auto_update_delay:
                if ready_on_time:
                    self._delayed_executer.multiply_sleep_duration(ASYNC_DELAY_MULTIPLIER_INCREASE)
                else:
                    self._delayed_executer.multiply_sleep_duration(ASYNC_DELAY_MULTIPLIER_DECREASE)
            self._join_setup_thread()
            if not ready_on_time:
                self._delayed_executer.enable()
        else:
            self._setup_next()
        self._set_next()
        if self._async_resets:
    ...
    def _setup_next(self) -> None:
        ...
        success = False
        while not success:
            next_scenario_bundle: ScenarioBundle = self._delayed_executer(
                self._env.scenario_iterator.__next__
            )
            logger.debug(f"Completed subtask 1/3: Retrieved next scenario bundle")
            next_ego_vehicle_simulation: EgoVehicleSimulation = self._delayed_executer(
            ...
            success = self._delayed_executer(
